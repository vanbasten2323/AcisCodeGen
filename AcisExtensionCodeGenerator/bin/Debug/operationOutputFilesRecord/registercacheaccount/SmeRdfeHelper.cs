...
namespace Microsoft.Cloud.Engineering.RdfeExtension
{
    ...
    public partial class SmeRdfeHelper
    {
        ...
        /// <param name="PublisherCode">Publisher Code</param>
        /// <returns>IAcisSMEOperationResponse</returns>
        public IAcisSMEOperationResponse DeletePublisher(string subscriptionId, string publisherCode)
        {
            return this.ExecuteManagementOperation(
                (mgmt, context) =>
                {
                    using (context())
                    {
                        ...
                        mgmt.DeletePublisher(subscriptionId, publisherCode);
                        PublisherFormatter formatter = new PublisherFormatter(this.Endpoint);

                        return formatter.FormatUpdatedPublisherCode(publisherCode); //TODO: run test on acis-test to see the success message.
                    }
                },
                    err => string.Format("Unable to delete publisher due to {0}.", err)); 
        }
        ...
        /// <summary>
        /// Register the cache account.
        /// </summary>
        /// <param name="accountName">Cache Account Name</param>
        /// <param name="currentKeyIndex">Cache Account Current Key Index</param>
        /// <param name="location">Location</param>
        /// <param name="stampName">Cache Account Stamp Name</param>
        /// <param name="isStampActive">Cache Account Is Active</param>
        /// <param name="typeOfCache">Cache Account Type of Cache</param>
        /// <returns>IAcisSMEOperationResponse</returns>
        public IAcisSMEOperationResponse RegisterCacheAccount(string accountName, string currentKeyIndex, string location, string stampName, string isStampActive, string typeOfCache)
        {
            return this.ExecuteAdministrationOperation(
                (admin, context) =>
                {
                    CacheType typeOfCacheVal;
                    int currentKeyIndexVal;
                    bool isStampActiveVal;

                    // convert to the non-string values
                    AcisSMEOperationResponse parseResponse = ParseCacheAccountArguments(typeOfCache, currentKeyIndex,
                        isStampActive, out typeOfCacheVal, out currentKeyIndexVal, out isStampActiveVal);

                    CacheAccountInput accountInput = new CacheAccountInput
                    {
                        AccountName = accountName,
                        CurrentKeyIndex = currentKeyIndexVal,
                        IsStampActive = isStampActiveVal,
                        Location = location,
                        StampName = stampName,
                        TypeOfCache = (int) typeOfCacheVal
                    };

                    using (context())
                    {
                        InsertUserAgentToOutgoingRequest();
                        CacheAccount account = admin.RegisterCacheAccount(accountInput);
                        CacheAccountFormatter formatter = new CacheAccountFormatter(this.Endpoint);
                        return formatter.FormatCacheAccount(account);
                    }
                },
                err => string.Format("Unable to register cache account due to {0}.", err)); // TODO: Refine the error message.
        }
        ...
        /// <summary>
        /// Unregister the cache account.
        /// </summary>
        /// <param name="accountname">Cache Account Name</param>
        /// <returns>IAcisSMEOperationResponse</returns>
        public IAcisSMEOperationResponse UnregisterCacheAccount(string accountname)
        {
            return this.ExecuteAdministrationOperation(
                (admin, context) =>
                {
                    ...
                    {
                        InsertUserAgentToOutgoingRequest();
                        admin.UnregisterCacheAccount(accountname);
                        return string.Format("Successfully unregisterd cache account with name '{0}'", accountname);
                    }
                },
			    err => string.Format("Unable to unregister cache account due to {0}.", err));
        }
        ...
        #region FCIR Operations
        public IAcisSMEOperationResponse DisableFCIRForFabricGeoId(string fabricGeoId)
        {
            return this.ExecuteAdministrationOperation(
                (admin, context) =>
                {
                    using (context())
                    {
                        admin.DisableFabricGeoIdForFCIR(fabricGeoId);
                    }

                    return string.Format("Disabled FCIR for Fabric GeoId {0} successfully.", fabricGeoId);
                },
            err => string.Format("Failed to disable FCIR for Fabric GeoId. {0}{1}", Environment.NewLine, err));
        }

        public IAcisSMEOperationResponse EnableFCIRForFabricGeoId(string fabricGeoId)
        {
            return this.ExecuteAdministrationOperation(
                (admin, context) =>
                {
                    using (context())
                    {
                        admin.EnableFabricGeoIdForFCIR(fabricGeoId);
                    }

                    return string.Format("Enabled FCIR for Fabric GeoId {0} successfully.", fabricGeoId);
                },
            err => string.Format("Failed to disable FCIR for Fabric GeoId. {0}{1}", Environment.NewLine, err));
        }
        #endregion
        ...
        /// <summary>
        /// Parses the cache account arguments.
        /// </summary>
        /// <param name="typeOfCache">The type of cache.</param>
        /// <param name="currentKeyIndex">Index of the current key.</param>
        /// <param name="isStampActive">The is stamp active.</param>
        /// <param name="typeOfCacheVal">The type of cache val.</param>
        /// <param name="currentKeyIndexVal">The current key index val.</param>
        /// <param name="isStampActiveVal">if set to <c>true</c> [is stamp active val].</param>
        /// <returns>
        /// The response, if errored.
        /// </returns>
        public static AcisSMEOperationResponse ParseCacheAccountArguments(string typeOfCache, string currentKeyIndex,
            string isStampActive, out CacheType typeOfCacheVal, out int currentKeyIndexVal, out bool isStampActiveVal)
        {
            currentKeyIndexVal = 0;
            isStampActiveVal = false;

            if (!Enum.TryParse(typeOfCache, out typeOfCacheVal))
            {
                throw new ArgumentException(string.Format("Unable to parse Type of Cache value '{0}' as a valid CacheType.", typeOfCache));
            }

            if (!int.TryParse(currentKeyIndex, out currentKeyIndexVal))
            {
                throw new ArgumentException(string.Format("Unable to parse Current Key value '{0}' as a valid integer.", currentKeyIndex));
            }

            if (!bool.TryParse(isStampActive, out isStampActiveVal))
            {
                throw new ArgumentException(string.Format("Unable to parse Is Stamp Active value '{0}' as a valid boolean.", isStampActive));
            }
